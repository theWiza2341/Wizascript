...
const RANT_DISABLED_CACHE_KEY = "wizascript.decktracker.rantDisabledCache";
...
export function registerDeckTrackerSettings(plugin) {
  ...
  // reflecting whatever the user last set the toggle to. Confirmed
  // acceptable to the user: doesn't update the SAME session it's
  // toggled in, only from the next page load/match onward, since our
  // cache is synced (see below) throughout the current session for
