...
// @updateURL    https://raw.githubusercontent.com/theWiza2341/Wizascript/refs/heads/dev/wizascript.user.js
// @downloadURL  https://raw.githubusercontent.com/theWiza2341/Wizascript/refs/heads/dev/wizascript.user.js
// @grant        GM_getValue
// @grant        GM_setValue
...
(() => {
  ...
  var SUITE_VERSION = "1.5.0.202609230426";
...
chrome: ${chromeStates[chromeIndex] ? chromeStates[chromeIndex].type : "?"}`;
  ...
  var EYE_TOTAL_HEIGHT = SHAPE.eyeHeight + SHAPE.spikeLength * 2;
  var EYE_TOTAL_WIDTH = SHAPE.eyeWidth + SHAPE.cornerLineLength * 2;
  var EYE_JITTER = [
    { dx: 0, dy: 0, scale: 1 },
  ...
  var STYLE_ID = "wiza-dt-titan-style";
  var CSS = `
...
`;
  function createEffect2(ctx) {
    ...
    function renderEyes() {
      ...
      computeEyePositions().forEach((pos) => {
        ...
        eye.style.setProperty("--eye-scale", (_a = pos.scale) != null ? _a : 1);
        if (withHalo) {
          const halo = document.createElement("div");
          halo.className = "wiza-titan-halo";
          eye.appendChild(halo);
        }
    ...
    function shatterEye(eyeDiv) {
      ...
      if (reducedMotion()) {
        eyeDiv.classList.remove("is-visible");
        return;
      }
    ...
    function startPupilShake() {
      ...
      shakeActive = true;
      const seeds = eyePupils.map((_, i) => i * 1.9);
      function tick(now) {
  ...
  function registerAnimation(def, source) {
  ...
  var GAME_END_ACTIONS = /* @__PURE__ */ new Set(["getVictory", "getDefeat", "getResult"]);
  function createLoader({ animations: animations2, settings: settings2, logger: logger4 }) {
    ...
    function isRelevantPlayer(playerId) {
      if (playerId === null || playerId === void 0) return false;
      const me = getRelevantPlayerId();
      if (me !== null && Number(playerId) === me) return true;
      return settings2.allowOpponent();
    }
      ...
      if (current) {
        ...
        if (curEffect && curEffect.isActive()) {
          if (current.id === anim.id && !current.ending) {
            logger4.log(anim.id, "start ignored - already playing.");
            return false;
          }
          if (!current.ending && !force && settings2.overlapPolicy() === "ignore") {
    ...
    function react(anim, kind) {
      ...
      if (effect && typeof effect.react === "function") {
        logger4.log(anim.id, `react: ${kind}`);
        effect.react(kind);
      }
    }
    ...
    function onGameEvent(event) {
      if (!event || !settings2.isMasterEnabled()) return;
      if (GAME_END_ACTIONS.has(event.action)) {
        logger4.log(null, `game end (${event.action}) - stopping all.`);
        stopAll();
        return;
      }
      animations2.forEach((anim) => {
        ...
        try {
          ...
          logger4.error(anim.id, "detector threw:", err);
        }
      });
    }
    const debugApi = {
