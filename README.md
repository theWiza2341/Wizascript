...
Cosmetic, staff-approved animations for DT cards, added a few at a time. So far there are two:
- **Titan:** while Constricting Darkness is active, the screen darkens and eyes gather around the board. They follow your cursor, flash red when the counter drops, and shatter when it ends.
- **The Barrier:** when it's played, the screen cracks and breaks open. It comes in two styles: the classic GIF, or an animated remake. DT Animations is registered as its own UnderScript plugin with its own settings page, so it works even with every other Wizascript feature turned off. By default an animation only plays for DTs you play. An experimental setting extends that to your opponent's DTs, and a second setting picks whether a newer DT replaces the current animation or is ignored. Each DT gets one toggle, and its options only show while it's enabled. See `packages/dt-animations/README.md` for how to add one.
...
assets/            runtime-fetched binary assets (GIFs etc.), never bundled - see packages/core/assets.js
